...
class ModelSkeleton:
  ...
  def _conv_bn_layer(
      ...
      conv_with_bias=True, stddev=0.001, decay=0.999):
    ...
    with tf.variable_scope(conv_param_name) as scope:
        ...
        if conv_param_name in cw:
          ...
          if (kernel_val.shape == 
            ...
            print ('Shape of the pretrained parameter of {} does not match, '
                'use randomly initialized parameter'.format(conv_param_name))
        else:
          print ('Cannot find {} in the pretrained model. Use randomly initialized '
                 'parameters'.format(conv_param_name))

      if use_pretrained_param:
        if mc.DEBUG_MODE:
          print ('Using pretrained model for {}'.format(conv_param_name))
      ...
      beta  = _variable_on_device('beta', [filters], beta_val,
                                  trainable=(not freeze))
      moving_mean = _variable_on_device('moving_mean', [1, 1, 1, filters], tf.zeros_initializer,
                                        trainable=False)
      moving_var = _variable_on_device('moving_var', [1, 1, 1, filters], tf.ones_initializer,
                                        trainable=False)
      self.model_params += [gamma, beta, moving_mean, moving_var]
      conv = tf.nn.conv2d(
      ...
      if mc.DATA_AUGMENTATION: #training
      ...
      else: #testing using moving mean and var
        conv = tf.nn.batch_normalization(
  ...
  def _conv_layer(
      ...
      freeze=False, xavier=False, relu=True, stddev=0.001, bias_init_val=0.0, init=False):
